...
public class MyAccount extends JPanel{
	...
	public MyAccount(JFrame mf, String getId, String getPwd, MemberDao md)
	{
		list = md.myAccount(getId, getPwd);
		...
		JLabel userId = new JLabel(list.getUserId() + " 님");
		...
		//이름 표시
		JTextField name = new JTextField(list.getName());
		...
		//생일 표시
		JTextField birth = new JTextField(list.getBirthDay().substring(0, 6));
		...
		//휴대폰 번호 표시
		String setPhone = list.getPhoneNumber();
		JTextField phone = new JTextField(list.getPhoneNumber());
		...
		//이메일 표시
		JTextField email = new JTextField(list.getEmail());
